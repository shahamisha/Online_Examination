...
<form method="post" id="updateExamineeFrm">
     ...
     <div class="form-group">
        ...
        <?php 
            $exmneCourse = $selExmne['s_id'];
            $selCourse = $conn->query("SELECT * FROM sub_master WHERE s_id='$exmneCourse' ")->fetch(PDO::FETCH_ASSOC);
         ?>
         <select class="form-control" name="exCourse">
           <option value="<?php echo $exmneCourse; ?>"><?php echo $selCourse['name']; ?></option>
           <?php 
             $selCourse = $conn->query("SELECT * FROM sub_master WHERE s_id='$exmneCourse' ");
